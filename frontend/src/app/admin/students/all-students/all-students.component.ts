  ...
  exportExcel() {
    const exportData: Partial<TableElement>[] =
      this.dataSource.filteredData.map((x) => ({
        ...
        // 'Mobile': x.mobile,
        'Email': x.email,
        'Date of Birth': formatDate(new Date(x.dateBirth), 'yyyy-MM-dd', 'en') || '',
      }));

    TableExportUtil.exportToExcel(exportData, 'students');
  }
...
export class ExampleDataSource extends DataSource<Students> {
  ...
  connect(): Observable<Students[]> {
    ...
    this.exampleDatabase.getAllStudents(); // Make sure this fetches data from backend
